<?php
	...
	$result = mysqli_query($con,"SELECT * FROM ratings WHERE type = 1 AND place = $game_id AND author = {$_SESSION['user_id']}");
	if (mysqli_num_rows($result)>=1) {
		$row = mysqli_fetch_assoc($result);
		if ($new_rating>0) {
			$result2 = mysqli_query($con,"UPDATE ratings SET rating = $new_rating, posted = CURRENT_TIMESTAMP WHERE type = 1 AND place = $game_id AND author = {$_SESSION['user_id']}");
		} else {
			$result2 = mysqli_query($con,"DELETE FROM ratings WHERE type = 1 AND place = $game_id AND author = {$_SESSION['user_id']}");
		}
	} else {
		if (!$result2 = mysqli_query($con,"INSERT INTO ratings (type,place,author,ip,rating) VALUES (1,$game_id,{$_SESSION['user_id']},'$ip',$new_rating)")) {
